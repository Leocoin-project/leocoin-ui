...
var sendPage = (function ($) {
    ...
    function toggleCoinControl(enable) {
        if (enable === undefined)
            enable = $(".show-coin-control").hasClass('active');
    ...
    function changeTransactionType(event) {
        ...
        $("#spend_LEOcoin").toggle(from_type === "private");

        $("#to_sdc").toggle(to_type === "public");
        $("#to_LEOcoin").toggle(to_type === "private");

        $("#to_balance").toggle(!main);

        // TODO: Fix coin control for LEO -> SDT
    ...
    function getTransactionType() {
        ...
        if (from_public)
            return +!to_public; // 0 (LEO_TO_LEO) or 1 (LEO_TO_ANON)
        else
            return 2 + to_public; // 2 (ANON_TO_ANON) or 3 (ANON_TO_LEO)
    }
